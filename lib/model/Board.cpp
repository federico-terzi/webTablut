...
const int LOAD_EMPTY = 0;
const int LOAD_WHITE = 1;
const int LOAD_BLACK = 2;
const int LOAD_KING = 3;

// This method initializes all the data structures based on the json string.
void Board::load_board(const int * raw_board, bool is_white) {
    this->is_white = is_white;
    for (int y = 0; y<DIM; y++) {
        for (int x = 0; x<DIM; x++) {
            if (raw_board[x+y*DIM] == LOAD_WHITE) {
                board[x][y] |= Pawn::White;
            } else if (raw_board[x+y*DIM] == LOAD_BLACK) {
                board[x][y] |= Pawn::Black;
            } else if (raw_board[x+y*DIM] == LOAD_KING) {
                board[x][y] |= Pawn::King;
